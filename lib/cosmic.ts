...
import type { Service, CaseStudy, Testimonial, SiteSettings } from '@/types'
...
export async function getSiteSettings(): Promise<SiteSettings | null> {
  try {
    ...
    if (hasStatus(error) && error.status === 404) {
      // Fallback: fetch first site-settings object
      try {
        const list = await cosmic.objects.find({ type: 'site-settings' }).depth(1)
        const objects = list.objects as SiteSettings[]
